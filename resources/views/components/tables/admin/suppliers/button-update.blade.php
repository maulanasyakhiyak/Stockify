...
<button type="button" id="drawer-update-button-product-{{ $supplier->id }}"
    data-drawer-target="drawer-update-product-{{ $supplier->id }}"
    data-drawer-show="drawer-update-product-{{ $supplier->id }}"
    aria-controls="drawer-update-product-{{ $supplier->id }}" data-drawer-placement="right"
...
<!-- Edit Product Drawer -->
<div id="drawer-update-product-{{ $supplier->id }}"
    ...
    <h5 id="drawer-label"
        ...
        Update Product</h5>
    <button type="button" data-drawer-dismiss="drawer-update-product-{{ $supplier->id }}"
        aria-controls="drawer-update-product-{{ $supplier->id }}"
    ...
    <form action="{{ route($routeUpdate,$supplier['id']) }}" data-form-update="{{ $supplier->id }}" method="POST" class="overflow-x-auto h-5/6" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="space-y-4">
            <div>
                ...
                <input type="text" name="name" id="name-{{ $supplier->id }}"
                    ...
                    placeholder="Type product name" value="{{ $supplier['name'] }}">
            ...
            <div>
                ...
                <input type="text" name="email" id="email-{{ $supplier->id }}"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                    placeholder="Type product email" value="{{ $supplier['email'] }}">
