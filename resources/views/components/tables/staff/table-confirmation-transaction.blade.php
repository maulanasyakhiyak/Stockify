        ...
        <div class="overflow-hidden shadow">
            <table class="w-full divide-y divide-gray-200 table-auto dark:divide-gray-600">
                ...
                <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                    @forelse ($stockTransaction->where('status', 'pending') as $item)
                        @php
                                    ...
                                    <div data-modal="body">
                                        ...
                                        <div
                                            ...
                                            <div class=" py-2 font-medium">
                                                <h3 class="font-normal text-base text-gray-800 dark:text-gray-400 mb-2">
                                                    Notes</h3>
                                                <textarea rows="4" name="notes" id="notes-transaction-{{$item->id}}"
                                                    ...
                                                    {{$item->status == 'pending' ? '' : 'readonly'}}>{{ $item->notes }}</textarea>
                                            </div>
                                        </div>
                                        <div class="flex justify-end border-t dark:border-gray-600 p-2 py-3 gap-1">
                                            @if ($item->status == 'pending')
                                                <form action="{{ route('reject_transaction', $item->id) }}"
                                                    method="post" data-id="{{$item->id}}" data-form="reject" id="reject-{{$item->id}}">
                                                    @csrf
                                                    <button type="submit"
                                                ...
                                                <form action="{{ route('confirm_transation', $item->id) }}"
                                                    method="post" data-id="{{$item->id}}" data-form="accept" id="accept-{{$item->id}}">
                                                    @csrf
                                                    <button type="submit"
                                            ...
                                            @else
                                                <button type="button" data-hide-modal="transaction-{{ $item['id'] }}"
                                                    class="px-4 py-2 text-sm font-medium focus:outline-none text-white bg-gray-700 hover:bg-gray-800 focus:ring-4 focus:ring-gray-300 rounded-lg dark:bg-gray-600 dark:hover:bg-gray-700 dark:focus:ring-gray-900">
                                                    Cancel
                                                </button>
                                            @endif
                                        </div>
