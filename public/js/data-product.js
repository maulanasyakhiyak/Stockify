...
async function addDataToModal(target) {
    ...
    // Mengambil data dari fungsi async
    let data = await getSelectedId(true);
    console.log(data);


    // Misalkan data adalah array, kita iterasi melalui data
    let $html = '';  // Inisialisasi string kosong untuk menyimpan HTML

    data.forEach((item) => {  // Pastikan kita iterasi dengan tanda kurung ()
        $html += `<p class="text-red-400">ID: ${item.name}</p>`;  // Akses 'name' dari item yang diiterasi
    });

    // Menghapus konten lama di modal_body
    $children.empty();

    // Menambahkan konten baru yang sudah diproses
